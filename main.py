...
def cut_merge_one_video(file):
    ...
    print(time_dur)
    for i in range(15):
        cut_file = f"cut/{file}_cut{i}.mp4"
        start = random.randint(0, int(time_dur) - 8)
        print(start)
        start_time = get_start_time(start)
        print(start_time)
        cut_cmd = cut_video(start_time, 8, file, cut_file)
        print(cut_cmd)
        execuateCmd(cut_cmd)
    os.chdir("cut")
...
def cut_merge_multi_videos():
    ...
    for file in os.listdir("."):
        if (file.endswith(".mp4")):
            print(file)
            cut_file = f"cut/{file}_cut.mp4"
            cut_cmd = cut_video("00:00", 3, file, cut_file)
            print(cut_cmd)
            execuateCmd(cut_cmd)
            print("over")
    os.chdir("cut")
    execuateCmd("cp ../../gen_merge_txt.sh .")
    execuateCmd("sh gen_merge_txt.sh")
    merge_cmd = merge_video("merge.mp4")
    print(merge_cmd)
    execuateCmd(merge_cmd)
